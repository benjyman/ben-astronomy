...
def combine_images_to_pdf(imagelist,pol,figname):
   ...
   for image in imagelist:
      ...
      print cmd
      os.system(cmd)
   if 'compare' in figname:
      cmd = '/usr/bin/montage compare*%s*_comp.jpg -mode concatenate -tile 3x5 %s' % (pol,figname)
   else:
...
if AAVS1:
   ...
   for pol in pol_list:
      ...
      #for ant_index,ant in enumerate(ant_list):
         ...
         power_pattern_ant_filename = '/mnt/md0/AAVS-1/beam_pattern_tests/data/AAVS1_power_pattern_linear_Ant%s_%s_theta_phi.npy' % (ant_name_randall,pol)
...
if plot_cal_sols_only:      
   ...
   for ant_name_randall_number in range(1,257):
      ...
      title = 'Antenna Gain Ampfor %s Y Pol' % (ant_name_randall)
   ...
   for line in lines:
      ...
      for pol in pol_list:
         ...
         for sun_az_randall_deg_index,sun_az_randall_deg in enumerate(sun_az_list):
            ...
            beam_power_ant_compare_pos = ant_compare_pattern_normalised[sun_phi_daniel_deg,sun_theta_daniel_deg]
            #beam_power_ant_compare_pos = ant_compare_pattern_normalised[sun_theta_daniel_deg,sun_phi_daniel_deg]
            #beam_power_ant2_pos = ant002_pattern[sun_phi_daniel_deg,sun_theta_daniel_deg]
         ...
         #amplitude_ant2_array = np.asarray(amplitude_ant2_list).astype(np.float)
         #amplitude_ratio_ant100_ant002_array = np.asarray(amplitude_ratio_ant100_ant002_list)
         #max_ant002 = np.amax(amplitude_ant2_array)
         #amplitude_ant_compare_array_norm = amplitude_ant_compare_array/max_ant002
      
         #print "sun_az_list"
         #print sun_az_list
         #print "sun_phi_daniel_deg_list"
         #print sun_phi_daniel_deg_list
         
         #print "sun_elev_list"
         #print sun_elev_list
         #print "sun_theta_daniel_deg_list"
         #print sun_theta_daniel_deg_list
         ...
         power_title = 'Beam power Ant%s at Sun position and cal sol amp %s Pol' % (ant_name_randall,pol)
         power_figname = 'compare_beam_amp_to_cal_sols_Ant%s_%s_pol.jpg' % (ant_name_randall,pol)
         plt.legend(loc=1)
...
if combine_to_pdf:
   ...
   combined_figname='compare_beam_amp_to_cal_sols_AAVS-1_%s_pol.pdf' % pol
   ...
   #image_list = output.split('\n')
   #combined_figname='antenna_power_patterns_AAVS-1_%s_pol.pdf' % pol
   #combine_images_to_pdf(image_list,pol,combined_figname)      
   #pol='Y'
   #output = subprocess.check_output('ls Power_*%s*.png'%pol, shell=True)
   #image_list = output.split('\n')
