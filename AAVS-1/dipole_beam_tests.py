...
compare_sols_to_beam = True
new_sun_pos_file = False
combine_to_pdf = True
pdf_compare_beam_amp = True
pdf_compare_beam_phase = True
pdf_compare_beam_amp_EDA = False
pdf_compare_beam_phase_EDA = False
...
if (plot_AAVS1_array_only):

   good_list = [1,10,24,25,30,36,37,40,47,60,70,76,78,79,103,109,112,129,134,140,143,145,151,154,164,178,179,186,187,189,201,202,216,224,252,255,256]
   bad_list = [49,54,64,93,94,110,117,167,203,218,232,233]

   antenna_name_list = range(1,257)
   antenna_position_filename = "/mnt/md0/AAVS-1/AAVS1_locations_final.txt"
   ...
   antenna_position_plot_figname = 'antenna_positions_aavs1_good_and_bad.png'
   ...
   for i, name in enumerate(antenna_name_list):
       if name in good_list:
          ax.annotate(str(name), (antenna_position_x_m[i],antenna_position_y_m[i]),size=5, color='green')
       elif name in bad_list:
          ax.annotate(str(name), (antenna_position_x_m[i],antenna_position_y_m[i]),size=5, color='red')  
       elif name==2:
          ax.annotate(str(name), (antenna_position_x_m[i],antenna_position_y_m[i]),size=5, color='blue')
       else:
...
if plot_rms_only:
   rms_phase_beam_list_Y_np_filename = "rms_phase_beam_list_Y.npy"
   rms_phase_cal_sols_list_Y_np_filename = "rms_phase_cal_sols_list_Y.npy"
   rms_phase_corrected_sols_list_Y_np_filename = "rms_phase_corrected_sols_list_Y.npy"
   rms_phase_beam_list_Y = np.load(rms_phase_beam_list_Y_np_filename)
   rms_phase_cal_sols_list_Y = np.load(rms_phase_cal_sols_list_Y_np_filename)   
   rms_phase_corrected_sols_list_Y = np.load(rms_phase_corrected_sols_list_Y_np_filename)   
  
     
   rms_phase_beam_list_Y[rms_phase_cal_sols_list_Y==0]=np.nan
   rms_phase_corrected_sols_list_Y[rms_phase_cal_sols_list_Y==0]=np.nan
   rms_phase_cal_sols_list_Y[rms_phase_cal_sols_list_Y==0]=np.nan
   rms_phase_beam_list_Y[rms_phase_beam_list_Y==0]=np.nan
   rms_phase_corrected_sols_list_Y[rms_phase_corrected_sols_list_Y==0]=np.nan
   
   #split into chunks of antennas 
   ant_index_list = range(1,257)
   #chunk 1
   fig, ax = plt.subplots()
   ax.plot(ant_index_list[0:75],rms_phase_beam_list_Y[0:75],label='beam phase rms Y pol')
   ax.plot(ant_index_list[0:75],rms_phase_cal_sols_list_Y[0:75],label='sol phase rms Y pol')
   ax.plot(ant_index_list[0:75],rms_phase_corrected_sols_list_Y[0:75],label='corr. sol phase rms Y pol')  
   title = "phase rms over 2 hr sun pass AAVS1 Y pol Chunk 1"
   figname = "rms_phase_2r_sun_pass_AAVS1_Y_pol_chunk01.png"   
   plt.legend(loc=1)
   plt.xlabel('Antenna index')
   plt.ylabel('rms of phase')
   plt.title(title)
   plt.savefig(figname)
   plt.close()         
   ...
   #chunk 2
   fig, ax = plt.subplots()
   ax.plot(ant_index_list[75:150],rms_phase_beam_list_Y[75:150],label='beam phase rms Y pol')
   ax.plot(ant_index_list[75:150],rms_phase_cal_sols_list_Y[75:150],label='sol phase rms Y pol')
   ax.plot(ant_index_list[75:150],rms_phase_corrected_sols_list_Y[75:150],label='corr. sol phase rms Y pol')  
   title = "phase rms over 2 hr sun pass AAVS1 Y pol Chunk 2"
   figname = "rms_phase_2r_sun_pass_AAVS1_Y_pol_chunk02.png"   
   plt.legend(loc=1)
   plt.xlabel('Antenna index')
   plt.ylabel('rms of phase')
   plt.title(title)
   ...
   #chunk 3
   fig, ax = plt.subplots()
   ax.plot(ant_index_list[150:256],rms_phase_beam_list_Y[150:256],label='beam phase rms Y pol')
   ax.plot(ant_index_list[150:256],rms_phase_cal_sols_list_Y[150:256],label='sol phase rms Y pol')
   ax.plot(ant_index_list[150:256],rms_phase_corrected_sols_list_Y[150:256],label='corr. sol phase rms Y pol')  
   title = "phase rms over 2 hr sun pass AAVS1 Y pol Chunk 3"
   figname = "rms_phase_2r_sun_pass_AAVS1_Y_pol_chunk03.png"   
   plt.legend(loc=1)
   plt.xlabel('Antenna index')
   plt.ylabel('rms of phase')
   plt.title(title)
...
def combine_images_to_pdf(imagelist,pol,figname):
   ...
   #pdf = FPDF('P', 'mm', 'A4')
   #pdf.add_page()
   #x,y,w,h, = None,None,-10,-10   
   #for image in imagelist[0:10]:
   #   if image != '':
   #      image_base = image.split('.')[0]
   #      print image_base
   #      comp_image = '%s_compressed.png' % image_base
   ...
   #      pdf.image(comp_image)
   #
   #pdf.output("%s" % figname, "F")
   ...
   #output = subprocess.check_output('ls *%s* ' % figname_base, shell=True)
   #pdfs = output.split('\n')

   #for pdf in pdfs:
   #   if pdf != '':
   #      print pdf
   #      #cmd = "mv %s %s.pdf" % (pdf,pdf)
   #      #print cmd
   #      #os.system(cmd)
   #      fixPdf(pdf)
   #      merger.append("%s" % pdf)

   #merger.write(figname)
...
if AAVS1:
   ...
   for pol in pol_list:
      ...
      print "normalisation factor is %s" % normalisation_factor
      ...
      if apply_normalisation:
         Ephi_complex_Ant002 = Ephi_complex_Ant002 * normalisation_factor
         Etheta_complex_Ant002 = Etheta_complex_Ant002 * normalisation_factor
...
if compare_sols_to_beam:
   ...
   #   lines = f.readlines()
   #for line in lines:
   rms_phase_cal_sols_list_X = []
   rms_phase_beam_list_X = []
   rms_phase_corrected_sols_list_X = []
   rms_phase_cal_sols_list_Y = []
   rms_phase_beam_list_Y = []
   rms_phase_corrected_sols_list_Y = []
   for ant_index in range(1,257):
      ...
      for pol in pol_list:
         ...
         #remove mean phase from sols and beams
         sun_ph_sols_list_mean = np.nanmean(sun_ph_sols_list)
         sun_ph_sols_list_mean_subtr = sun_ph_sols_list - sun_ph_sols_list_mean
         phase_ant_compare_array_deg_mean = np.nanmean(phase_ant_compare_array_deg)
         phase_ant_compare_array_deg_mean_subtr = phase_ant_compare_array_deg - phase_ant_compare_array_deg_mean
            
         rms_phase_cal_sols = np.sqrt(np.mean(np.square(sun_ph_sols_list_mean_subtr)))
         rms_phase_beam = np.sqrt(np.mean(np.square(phase_ant_compare_array_deg_mean_subtr)))
         if pol=='X':
            rms_phase_cal_sols_list_X.append(rms_phase_cal_sols)
            rms_phase_beam_list_X.append(rms_phase_beam)            
         else:
            rms_phase_cal_sols_list_Y.append(rms_phase_cal_sols)
            rms_phase_beam_list_Y.append(rms_phase_beam)    
 
         if correct_phases:
            corrected_phase_sols = sun_ph_sols_list_mean_subtr + phase_ant_compare_array_deg_mean_subtr
            rms_phase_corrected_sols = np.sqrt(np.mean(np.square(corrected_phase_sols)))
            if pol=='X':
               rms_phase_corrected_sols_list_X.append(rms_phase_corrected_sols)
            else:
               rms_phase_corrected_sols_list_Y.append(rms_phase_corrected_sols)
         ...
         ax.plot(datetimes,sun_amp_sols_list,label='ant%s sol /max(ant002 sol) %s Pol' % (ant_name_randall,pol))
         ax.xaxis.set_major_formatter(formatter)  
         ax.set_ylim(-0.1,1.2)
         #ax.plot(x,amplitude_ratio_ant100_ant002_array,label='ant100/ant002')
         ...
         #ax.plot(datetimes,sun_ph_sols_list,label='ant%s sol phase %s Pol' % (ant_name_randall,pol))
         if correct_phases:
            ax.plot(datetimes,corrected_phase_sols,label='ant%s corrected phase sol %s Pol' % (ant_name_randall,pol))
            ax2 = ax.twinx()
            ax2.plot(datetimes,amplitude_ant_compare_array,label='ant%s beam AMP %s Pol' % (ant_name_randall,pol),color='red')
            ax2.set_ylim(0,1)
            ax2.legend(loc=4)
         ax.plot(datetimes,phase_ant_compare_array_deg_mean_subtr,label='ant%s beam model phase ref Ant002' % ant_name_randall)
         ax.plot(datetimes,sun_ph_sols_list_mean_subtr,label='ant%s sol phase %s Pol' % (ant_name_randall,pol))
         ax.xaxis.set_major_formatter(formatter)
         ax.set_ylim(-60,60)
         ax.legend(loc=1)
         #ax.ylabel('Phase (deg)')
   ...
   rms_phase_cal_sols_list_Y_np_filename = "rms_phase_cal_sols_list_Y.npy"
   rms_phase_corrected_sols_list_Y_np_filename = "rms_phase_corrected_sols_list_Y.npy"
   np.save(rms_phase_beam_list_Y_np_filename,rms_phase_beam_list_Y)
   print "saved %s" % rms_phase_beam_list_Y_np_filename
   np.save(rms_phase_cal_sols_list_Y_np_filename,rms_phase_cal_sols_list_Y)
   print "saved %s" % rms_phase_cal_sols_list_Y_np_filename
   np.save(rms_phase_corrected_sols_list_Y_np_filename,rms_phase_corrected_sols_list_Y)
   print "saved %s" % rms_phase_corrected_sols_list_Y_np_filename   
