...
def generate_sbatch_script_CenA(obsid_list,ms_dir_list,n_selfcals,download=False,model_cal=False):
   ...
   #do qa again after model cal then proceed
      ...
      for selfcal in range(0,int(n_selfcals*2.),2):
         ...
         out_filename = generate_wsclean_image(obsid_list,ms_dir_list,out_image_name_base='test1',wsclean_options=wsclean_options_1,dest_dir='/fred/oz048/bmckinle/ATeam/CenA/image4',self_cal_number=int(selfcal/2.))
   
         cmd = 'jid%s=$(sbatch %s) \n' % (selfcal,out_filename)
         cmd_list.append(cmd)
         
         out_filename = generate_selfcal(obsid_list,ms_dir_list,calibrate_options=calibrate_options_1,self_cal_number=int(selfcal/2.),dest_dir='/fred/oz048/bmckinle/ATeam/CenA/image4')
