...
fine_chan_width_Hz =  (400./512.) / 27. * 1.0e6#28935 #Hz should be:(400./512.)*1.0e6 
fine_chan_width_MHz = fine_chan_width_Hz / 1.0e6

#for casa ms it appears to be just 28 kHz
#fine_chan_width_Hz =  28000

#test - halve the coarse chan centre offset and see if that does anything - it doesnt
#coarse_chan_centre_offset_MHz = 0.46296 / 2.  #fine_chan_width_Hz * 16
...
def calculate_freq_MHz_fine_chan_array(EDA2_chan_list,fine_chans_per_EDA2_chan=27):
   #each EDA_chan has 32 fine chans but we flag 2 at start and 3 at end (oversampled) so 27 used
   freq_MHz_fine_chan_index_array = np.arange(int(len(EDA2_chan_list)*fine_chans_per_EDA2_chan),dtype="float32")
   #print(EDA2_chan_list)
   start_index = 0
   end_index = start_index + fine_chans_per_EDA2_chan
   for EDA2_chan_index,EDA2_chan in enumerate(EDA2_chan_list):
      ...
      freq_MHz_fine_chan_index_array[start_index:end_index] = freq_MHz_fine_chan_sub_array
      start_index += fine_chans_per_EDA2_chan
      end_index += fine_chans_per_EDA2_chan
      
   return freq_MHz_fine_chan_index_array
      
#EDA2_chan_list = [77,78,79,80,81]
#freq_MHz_fine_chan_index_array = calculate_freq_MHz_fine_chan_array(EDA2_chan_list)
...
#freq_MHz_fine_chan_sub_array = calculate_freq_MHz_fine_chan_subarray(77)
#print(freq_MHz_fine_chan_sub_array)
#sys.exit()
   

   
def simulate_eda2_with_complex_beams(EDA2_chan_list,lst_hrs_list,nside=512,antenna_layout_filename='/md0/code/git/ben-astronomy/EoR/ASSASSIN/ant_pos_eda2_combined_on_ground_sim.txt',plot_from_saved=False,EDA2_obs_time_list=[],sim_unity=True,sim_pt_source=False,check_figs=False,fine_chan=False,base_freq_index_offset=0):
   ...
   #make freq calculations consistent across functions
   ...
   #   #dont use the first 16 fine chans (if starting at 50 MHz) because beams havent been made  below 50 MHz and we dont want to extrapolate
   ...
   for EDA2_chan_index,EDA2_chan in enumerate(EDA2_chan_list):
      ...
      for freq_MHz_fine_chan_index,freq_MHz_fine_chan in enumerate(freq_MHz_fine_chan_subarray):
         ...
         number = (freq_MHz+(fine_chan_width_MHz/2.))*1000000
         print(number)
         freq_Hz_string = "%d" % ((np.round(number/5.) * 5.) - (5*EDA2_chan_index))
         print(freq_Hz_string)
         wavelength = 300./freq_MHz
...
def write_to_miriad_vis(EDA2_chan_list,lst_hrs_list,EDA2_obs_time_list,antenna_layout_filename='/md0/code/git/ben-astronomy/EoR/ASSASSIN/ant_pos_eda2_combined_on_ground_sim.txt',input_sky="gsm",check_figs=False,fine_chan=False,base_freq_index_offset=0):
   #seems that for each coarse chan you offset from the base of 50 MHz you end up having to subtract 5 Hz from the fine chan freq - I don't know why, going
   #to have to just hard code it for now. 
   base_freq_MHz = EDA2_chan_list[0] * (400./512.)
   ...
   for EDA2_chan_index,EDA2_chan in enumerate(EDA2_chan_list):
      ...
      for freq_MHz_fine_chan_index,freq_MHz_fine_chan in enumerate(freq_MHz_fine_chan_subarray):
         ...
         uv['restfreq'] = 0.0
         uv['nschan'] = NFFT
         uv['sdf'] = fine_chan_width_Hz/1000000000. #FREQ_GHz[1]-FREQ_GHz[0]
         uv['corr'] = 0.0 
         uv['freq'] = FREQ_GHz[0]
...
def calibrate_with_complex_beam_model_fine_chan(EDA2_chan_list,lst_list=[],plot_cal=False,uv_cutoff=0,EDA2_data=True,sim_only_EDA2=[],run_aoflagger=True,rfi_strategy_name='/md0/code/git/ben-astronomy/EoR/ASSASSIN/rfi_strategy_new.rfis',base_freq_index_offset=0,model_dir = "sims/",casa_calibrate=False):
   ...
   for EDA2_chan_index,EDA2_chan in enumerate(EDA2_chan_list):  
      ...
      for EDA2_obs_time_index,EDA2_obs_time in enumerate(obs_time_list):
         ...
         for freq_MHz_fine_chan_index,freq_MHz_fine_chan in enumerate(freq_MHz_fine_chan_subarray):
            if (EDA2_chan_index==0 and math.isclose(base_freq_MHz,50.0)):
               ...
               print("skipping first 14 chans because base_freq_MHz is %s" % base_freq_MHz)
      ...
      #cmd = "wsclean -name %s -size %s %s -multiscale -channels-out 32 -weight briggs 0 -niter 500 -scale %s -pol xx,yy -data-column MODEL_DATA  %s " % (test_image_name+"_model",wsclean_imsize,wsclean_imsize,wsclean_scale,concat_ms_name)
      #print(cmd)
      #os.system(cmd)
      ...
      if casa_calibrate:
         casa_cal_table_name = 'complex_beam_test_calibrate_sols_%03d_casa.bcal' % EDA2_chan
         casa_cmd_filename = '%s/casa_bandpass.sh' % EDA2_chan
      
         cmd = "rm -rf %s %s" % (casa_cal_table_name,casa_cmd_filename)
         print(cmd)
         os.system(cmd)
            
         cmd = "bandpass(vis='%s',caltable='%s')" % (concat_ms_name,casa_cal_table_name)
         ...
         casa_cmd_filename = '%s/casa_applycal.sh' % EDA2_chan
      
         cmd = "rm -rf %s" % (casa_cmd_filename)
         print(cmd)
         os.system(cmd)
            
         cmd = "applycal(vis='%s',gaintable='%s')" % (concat_ms_name,casa_cal_table_name)
...
#Calibrate creates a new CONCAT ms each time!!! So need to reverse the individual time mss not jut the concat!!!!!!!!!
def reverse_ms_fine_chans(EDA2_chan_list=[],lst_list=[],base_freq_index_offset=0,copy=False):
   base_freq_MHz = EDA2_chan_list[0] * (400./512.) 
   fine_chans_per_EDA2_chan = 27
   ...
   #freq_MHz_fine_chan_subarray = calculate_freq_MHz_fine_chan_subarray(EDA2_chan)
   for EDA2_chan_index,EDA2_chan in enumerate(EDA2_chan_list): 
      print("reversing EDA2_chan %s" % EDA2_chan)
      #dont even need any of this fine chan stuff!
      #freq_MHz = 400./512.*float(EDA2_chan)
      #if (EDA2_chan_index==0 and math.isclose(base_freq_MHz,50.0)):
      #   fine_chans_per_EDA2_chan = 13
      #   freq_MHz_fine_chan_subarray = freq_MHz_fine_chan_subarray[14:]
      ...
      for EDA2_obs_time_index,EDA2_obs_time in enumerate(obs_time_list):
         ...
         if copy:
            print("making copy of %s" % eda2_ms_name)
            #copy_name = "%s/orig_copy_concat_chan_%s.ms" % (EDA2_chan,EDA2_chan)
            copy_name = "%s/%s_%s_eda2_ch32_ant256_midday_avg8140_orig_copy.ms" % (EDA2_chan,EDA2_obs_time[0:8],EDA2_obs_time[9:15])
            cmd = "cp -r %s %s" % (eda2_ms_name,copy_name)
            print(cmd)
            os.system(cmd)
...
def extract_autos_from_ms_complex(EDA2_chan_list=[],lst_list=[],uvdist_thresh_lambda=0.5,unity_vis_dir="sims/",n_fine_chans_included=27,base_freq_index_offset=0):
   ...
   nonauto_array_fine_chan = np.empty([len(EDA2_chan_list),fine_chans_per_EDA2_chan,max_n_timesteps])
   nonauto_array_fine_chan[:] = np.nan

   auto_array_filename_fine_chan = "auto_array_eda2_fine_chan.npy"
   nonauto_array_filename_fine_chan = "nonauto_array_eda2_fine_chan.npy"
   ...
   for EDA2_chan_index,EDA2_chan in enumerate(EDA2_chan_list): 
      ...
      for timestep in range(n_timesteps_eda2):
         ...
         for freq_MHz_fine_chan_index,freq_MHz_fine_chan in enumerate(freq_MHz_fine_chan_subarray):
            print(freq_MHz_fine_chan_index)
            #freq_MHz_fine_chan = freq_MHz_fine_chan_subarray[fine_chan_included]
            print(freq_MHz_fine_chan)
            
            if (EDA2_chan_index==0 and math.isclose(base_freq_MHz,50.0)):
               fine_chan_index = freq_MHz_fine_chan_index+16
            else:
               fine_chan_index = freq_MHz_fine_chan_index+2
            ...
            non_auto_indices = np.arange(200) + 1
            ...
            auto = eda2_autos_array_fine_chan[0,3]
            auto_array_fine_chan[EDA2_chan_index,freq_MHz_fine_chan_index,timestep] = auto
            
            eda2_nonautos_array_fine_chan = timestep_data_fine_chan[non_auto_indices,:]
            #nonauto = np.nanmean(eda2_nonautos_array_fine_chan[:,0])
            nonauto = eda2_nonautos_array_fine_chan[20,0]
            nonauto_array_fine_chan[EDA2_chan_index,freq_MHz_fine_chan_index,timestep] = nonauto
   ...
   np.save(nonauto_array_filename_fine_chan,nonauto_array_fine_chan)  
   print("saved %s" % nonauto_array_filename_fine_chan)        
            
def extract_global_signal_from_ms_complex(EDA2_chan_list=[],lst_list=[],uvdist_thresh_lambda=0.5,unity_vis_dir="sims/",n_fine_chans_included=27,base_freq_index_offset=0,ms_column="CORRECTED_DATA",max_deviations=5):
   base_freq_MHz = EDA2_chan_list[0] * (400./512.)
   n_pol = 2  #just look at x and y (real) for now
   fine_chans_per_EDA2_chan = 27
   #max umber of timesteps is set to eight 
   max_n_timesteps = 8
   ...
   t_sky_K_array_filename = "t_sky_K_array_eda2_%s.npy" % ms_column
   t_sky_error_K_array_filename = "t_sky_error_K_array_eda2_%s.npy" % ms_column
   t_sky_K_array_filename_flagged = "t_sky_K_array_eda2_flagged_%s.npy" % ms_column
   t_sky_error_K_array_filename_flagged = "t_sky_error_K_array_eda2_flagged_%s.npy" % ms_column
   
   t_sky_K_array_fine_chan = np.empty([len(EDA2_chan_list),fine_chans_per_EDA2_chan,max_n_timesteps,n_pol])
   t_sky_error_K_array_fine_chan = np.empty([len(EDA2_chan_list),fine_chans_per_EDA2_chan,max_n_timesteps,n_pol])
   t_sky_K_array_flagged_fine_chan = np.empty([len(EDA2_chan_list),fine_chans_per_EDA2_chan,max_n_timesteps,n_pol])
   t_sky_error_K_array_flagged_fine_chan = np.empty([len(EDA2_chan_list),fine_chans_per_EDA2_chan,max_n_timesteps,n_pol])
   ...
   t_sky_K_array_filename_fine_chan = "t_sky_K_array_eda2_fine_chan_%s.npy" % ms_column
   t_sky_error_K_array_filename_fine_chan = "t_sky_error_K_array_eda2_fine_chan_%s.npy" % ms_column
   t_sky_K_array_filename_flagged_fine_chan = "t_sky_K_array_eda2_flagged_fine_chan_%s.npy" % ms_column
   t_sky_error_K_array_filename_flagged_fine_chan = "t_sky_error_K_array_eda2_flagged_fine_chan_%s.npy" % ms_column 
   ...
   for EDA2_chan_index,EDA2_chan in enumerate(EDA2_chan_list): 
      ...
      eda2_ms_table = table(av_ms_name,readonly=True)
      eda2_data_complex = get_data(eda2_ms_table, col=ms_column)
      n_vis_eda2 = eda2_data_complex.shape[0]
      ...
      for timestep in range(n_timesteps_eda2):
         ...
         for freq_MHz_fine_chan_index,freq_MHz_fine_chan in enumerate(freq_MHz_fine_chan_subarray):
            ...
            wavelength = c / (freq_MHz_fine_chan*1e6)
            print(wavelength)
               
            unity_ms_name = "%sunity_%0.3f.ms" % (unity_vis_dir,freq_MHz_fine_chan)
            ...
            ##plt.ylim([0, 3.5])
            #fig_name= "test_unity_response_vs_uvdist_%0.3f_MHz_%s_pol.png" % (freq_MHz_fine_chan,"x")
            #figmap = plt.gcf()
            #figmap.savefig(fig_name)
            #plt.close()
            #print("saved %s" % fig_name)
            ...
            #just looking at real,x and y for now
            ...
            common_unity_data_complex_freq_av_y = common_unity_data_sorted[:,0,3]
            common_unity_data_complex_freq_av_y_real=common_unity_data_complex_freq_av_y.real
            ...
            common_unity_data_complex_freq_av_y_real_sorted_no_auto = common_unity_data_complex_freq_av_y_real[uvdist_array_unity_lambda>0]
            common_unity_data_complex_freq_av_y_real_sorted = common_unity_data_complex_freq_av_y_real_sorted_no_auto[uvdist_array_unity_inds]
                  
            if (EDA2_chan_index==0 and math.isclose(base_freq_MHz,50.0)):
               fine_chan_index = freq_MHz_fine_chan_index+16
               print("base freq is %s, setting fine_chan_index to %s " % (base_freq_MHz,fine_chan_index))
            else:
               fine_chan_index = freq_MHz_fine_chan_index+2
            timestep_data_fine_chan = timestep_data[:,fine_chan_index,:]
            base_name = "%s_timestep_%s_chan_%03d_fine_%02d" % (first_obstime,timestep,EDA2_chan,fine_chan_index)
            ...
            common_eda2_data_complex_y = common_eda2_data_sorted[:,3]
            common_eda2_data_complex_y_real=common_eda2_data_complex_y.real
            ...
            common_eda2_data_complex_y_real_sorted_no_auto = common_eda2_data_complex_y_real[uvdist_array_eda2_lambda>0]
            common_eda2_data_complex_y_real_sorted = common_eda2_data_complex_y_real_sorted_no_auto[uvdist_array_eda2_inds]
            common_eda2_data_complex_y_real_sorted_cut = common_eda2_data_complex_y_real_sorted[0:n_baselines_included_eda2]           
            ...
            common_unity_data_complex_y_real_sorted_cut = common_unity_data_complex_freq_av_y_real_sorted[0:n_baselines_included_eda2]
            
            #convert zeros to nan (there is something fishy with the y beams that puts some zeros in there)
            common_unity_data_complex_x_real_sorted_cut_zero_indices = np.where(np.isclose(common_unity_data_complex_x_real_sorted_cut,0.0))
            common_unity_data_complex_x_real_sorted_cut[common_unity_data_complex_x_real_sorted_cut_zero_indices] = np.nan
            common_unity_data_complex_y_real_sorted_cut_zero_indices = np.where(np.isclose(common_unity_data_complex_y_real_sorted_cut,0.0))
            common_unity_data_complex_y_real_sorted_cut[common_unity_data_complex_y_real_sorted_cut_zero_indices] = np.nan

            #remove the indices where common_unity_data_complex_y_real_sorted_cut is nan from common_unity_data_complex_y_real_sorted_cut and common_eda2_data_complex_y_real_sorted_cut
            common_unity_data_complex_y_real_sorted_cut_nonnan_indices = np.where(~np.isnan(common_unity_data_complex_y_real_sorted_cut))
            common_unity_data_complex_y_real_sorted_cut = common_unity_data_complex_y_real_sorted_cut[common_unity_data_complex_y_real_sorted_cut_nonnan_indices]
            common_eda2_data_complex_y_real_sorted_cut = common_eda2_data_complex_y_real_sorted_cut[common_unity_data_complex_y_real_sorted_cut_nonnan_indices]
            uvdist_array_eda2_lambda_sorted_cut_y = uvdist_array_eda2_lambda_sorted_cut[common_unity_data_complex_y_real_sorted_cut_nonnan_indices]
            uvdist_array_unity_lambda_sorted_cut_y = uvdist_array_unity_lambda_sorted_cut[common_unity_data_complex_y_real_sorted_cut_nonnan_indices]
            
            #there can also be zeros in the data (model column for y pol), so repeat for eda2 data:
            common_eda2_data_complex_y_real_sorted_cut_zero_indices = np.where(np.isclose(common_eda2_data_complex_y_real_sorted_cut,0.0))
            common_eda2_data_complex_y_real_sorted_cut[common_eda2_data_complex_y_real_sorted_cut_zero_indices] = np.nan
            common_eda2_data_complex_y_real_sorted_cut_nonnan_indices = np.where(~np.isnan(common_eda2_data_complex_y_real_sorted_cut))
            common_eda2_data_complex_y_real_sorted_cut = common_eda2_data_complex_y_real_sorted_cut[common_eda2_data_complex_y_real_sorted_cut_nonnan_indices]
            common_unity_data_complex_y_real_sorted_cut = common_unity_data_complex_y_real_sorted_cut[common_eda2_data_complex_y_real_sorted_cut_nonnan_indices]
            uvdist_array_eda2_lambda_sorted_cut_y = uvdist_array_eda2_lambda_sorted_cut_y[common_eda2_data_complex_y_real_sorted_cut_nonnan_indices]
            uvdist_array_unity_lambda_sorted_cut_y = uvdist_array_unity_lambda_sorted_cut_y[common_eda2_data_complex_y_real_sorted_cut_nonnan_indices]
             
            #x
            #remove the indices where common_unity_data_complex_y_real_sorted_cut is nan from common_unity_data_complex_y_real_sorted_cut and common_eda2_data_complex_y_real_sorted_cut
            common_unity_data_complex_x_real_sorted_cut_nonnan_indices = np.where(~np.isnan(common_unity_data_complex_x_real_sorted_cut))
            common_unity_data_complex_x_real_sorted_cut = common_unity_data_complex_x_real_sorted_cut[common_unity_data_complex_x_real_sorted_cut_nonnan_indices]
            common_eda2_data_complex_x_real_sorted_cut = common_eda2_data_complex_x_real_sorted_cut[common_unity_data_complex_x_real_sorted_cut_nonnan_indices]
            uvdist_array_eda2_lambda_sorted_cut_x = uvdist_array_eda2_lambda_sorted_cut[common_unity_data_complex_x_real_sorted_cut_nonnan_indices]
            uvdist_array_unity_lambda_sorted_cut_x = uvdist_array_unity_lambda_sorted_cut[common_unity_data_complex_x_real_sorted_cut_nonnan_indices]
            
            #there can also be zeros in the data (model column for y pol), so repeat for eda2 data:
            common_eda2_data_complex_x_real_sorted_cut_zero_indices = np.where(np.isclose(common_eda2_data_complex_x_real_sorted_cut,0.0))
            common_eda2_data_complex_x_real_sorted_cut[common_eda2_data_complex_x_real_sorted_cut_zero_indices] = np.nan
            common_eda2_data_complex_x_real_sorted_cut_nonnan_indices = np.where(~np.isnan(common_eda2_data_complex_x_real_sorted_cut))
            common_eda2_data_complex_x_real_sorted_cut = common_eda2_data_complex_x_real_sorted_cut[common_eda2_data_complex_x_real_sorted_cut_nonnan_indices]
            common_unity_data_complex_x_real_sorted_cut = common_unity_data_complex_x_real_sorted_cut[common_eda2_data_complex_x_real_sorted_cut_nonnan_indices]
            uvdist_array_eda2_lambda_sorted_cut_x = uvdist_array_eda2_lambda_sorted_cut_x[common_eda2_data_complex_x_real_sorted_cut_nonnan_indices]
            uvdist_array_unity_lambda_sorted_cut_x = uvdist_array_unity_lambda_sorted_cut_x[common_eda2_data_complex_x_real_sorted_cut_nonnan_indices]
            
            #print(uvdist_array_eda2_lambda_sorted_cut.shape)
            #print(common_eda2_data_complex_x_real_sorted_cut.shape)
            #plot the expected unity response vs baseline length
            plt.clf()
            plt.scatter(uvdist_array_eda2_lambda_sorted_cut_x,common_eda2_data_complex_x_real_sorted_cut)            
            map_title="Unity_response_vs_uvdist" 
            plt.xlabel("UV distance (wavelengths)")
            plt.ylabel("Real component of visibility X pol (Jy)")
            plt.legend(loc=1)
            #plt.text(x_pos, y_pos, fit_string)
            #plt.ylim([0, 3.5])
            fig_name= "eda2_response_vs_uvdist_%0.3f_MHz_%s_pol_%s.png" % (freq_MHz_fine_chan,"x",ms_column)
            figmap = plt.gcf()
            figmap.savefig(fig_name)
            plt.close()
            print("saved %s" % fig_name)
            ...
            #plot the expected unity response vs baseline length
            plt.clf()
            plt.scatter(uvdist_array_unity_lambda_sorted_cut_x,common_unity_data_complex_x_real_sorted_cut)            
            map_title="Unity_response_vs_uvdist" 
            plt.xlabel("UV distance (wavelengths)")
            plt.ylabel("Real component of visibility X pol (Jy)")
            plt.legend(loc=1)
            #plt.text(x_pos, y_pos, fit_string)
            #plt.ylim([0, 3.5])
            fig_name= "unity_response_vs_uvdist_%0.3f_MHz_%s_pol.png" % (freq_MHz_fine_chan,"x")
            figmap = plt.gcf()
            figmap.savefig(fig_name)
            plt.close()
            print("saved %s" % fig_name)
            ...
            #y pol
            plt.clf()
            plt.scatter(uvdist_array_eda2_lambda_sorted_cut_y,common_eda2_data_complex_y_real_sorted_cut)            
            map_title="EDA2_response_vs_uvdist" 
            plt.xlabel("UV distance (wavelengths)")
            plt.ylabel("Real component of visibility Y pol (Jy)")
            plt.legend(loc=1)
            #plt.text(x_pos, y_pos, fit_string)
            #plt.ylim([0, 3.5])
            fig_name= "eda2_response_vs_uvdist_%0.3f_MHz_%s_pol_%s.png" % (freq_MHz_fine_chan,"y",ms_column)
            figmap = plt.gcf()
            figmap.savefig(fig_name)
            plt.close()
            print("saved %s" % fig_name)
            ...
            #plot the expected unity response vs baseline length
            plt.clf()
            plt.scatter(uvdist_array_unity_lambda_sorted_cut_y,common_unity_data_complex_y_real_sorted_cut)            
            map_title="Unity_response_vs_uvdist" 
            plt.xlabel("UV distance (wavelengths)")
            plt.ylabel("Real component of visibility Y pol (Jy)")
            plt.legend(loc=1)
            #plt.text(x_pos, y_pos, fit_string)
            #plt.ylim([0, 3.5])
            fig_name= "unity_response_vs_uvdist_%0.3f_MHz_%s_pol.png" % (freq_MHz_fine_chan,"y")
            figmap = plt.gcf()
            figmap.savefig(fig_name)
            plt.close()
            print("saved %s" % fig_name)
            ...
            if (np.count_nonzero(~np.isnan(common_eda2_data_complex_x_real_sorted_cut))>2 and np.count_nonzero(~np.isnan(common_unity_data_complex_x_real_sorted_cut))>2):
               ...
               print("t_sky_K %s x pol is %0.4E +/- %0.04f K" % (ms_column,t_sky_K,t_sky_error_K))
               ...
               print(common_unity_data_complex_x_real_sorted_cut.shape)
               print(common_eda2_data_complex_x_real_sorted_cut.shape)
               ...
               plt.ylabel("Real component of visibility X pol (Jy)")
               plt.legend(loc=1)
               #optimise for chan 079
               x_pos = 1100
               y_pos = 0.5e7
               plt.text(x_pos, y_pos, fit_string)
               #plt.ylim([-0.25e7, 1.5e7])
               #plt.xlim([1000, 3000])
               fig_name= "x_y_OLS_%s_%s_pol_%s.png" % (base_name,"x",ms_column)
               figmap = plt.gcf()
               figmap.savefig(fig_name)
               plt.close()
               print("saved %s" % fig_name)
            ...
            if (EDA2_chan_index==0 and math.isclose(base_freq_MHz,50.0)):
               freq_MHz_fine_chan_index += 14
               print("base freq is %s, setting freq_MHz_fine_chan_index to %s " % (base_freq_MHz,freq_MHz_fine_chan_index))
            t_sky_K_array_fine_chan[EDA2_chan_index,freq_MHz_fine_chan_index,timestep,0] = t_sky_K
            t_sky_error_K_array_fine_chan[EDA2_chan_index,freq_MHz_fine_chan_index,timestep,0] = t_sky_error_K
            
            if (np.count_nonzero(~np.isnan(common_eda2_data_complex_x_real_sorted_cut))>2 and np.count_nonzero(~np.isnan(common_unity_data_complex_x_real_sorted_cut))>2):
            
               ###FLAGGING bit 
               #max_deviations = 5 # set in function def
               ...
               if (np.count_nonzero(~np.isnan(common_eda2_data_complex_x_real_sorted_cut_flagged))>2 and np.count_nonzero(~np.isnan(common_unity_data_complex_x_real_sorted_cut_missing))>2):
                  ...
                  print("t_sky_K %s x pol flagged is %0.4E +/- %0.04f K" % (ms_column,t_sky_K_flagged,t_sky_error_K_flagged))
                  ...
                  #plt.text(x_pos, y_pos, fit_string)
                  #plt.ylim([0, 3.5])
                  fig_name= "x_y_OLS_%s_%s_pol_flagged_%s.png" % (base_name,"x",ms_column)
                  figmap = plt.gcf()
                  figmap.savefig(fig_name)
                  plt.close()
                  print("saved %s" % fig_name)
            ...
            t_sky_K_array_flagged_fine_chan[EDA2_chan_index,freq_MHz_fine_chan_index,timestep,0] = t_sky_K_flagged
            t_sky_error_K_array_flagged_fine_chan[EDA2_chan_index,freq_MHz_fine_chan_index,timestep,0] = t_sky_error_K_flagged
      

      
            #y pol
            if (np.count_nonzero(~np.isnan(common_eda2_data_complex_y_real_sorted_cut))>2 and np.count_nonzero(~np.isnan(common_unity_data_complex_y_real_sorted_cut))>2):
               model = sm.OLS(common_eda2_data_complex_y_real_sorted_cut, common_unity_data_complex_y_real_sorted_cut,missing='drop')
               results = model.fit()
               parameters = results.params
               #print parameters
               t_sky_K = parameters[0]
               ...
               print("t_sky_K %s y pol is %0.4E +/- %0.04f K" % (ms_column,t_sky_K,t_sky_error_K))
               fit_string = "y=%0.1fx" % t_sky_K         #t_sky_K=%0.6f K" % (t_sky_jy,t_sky_K)
               print(fit_string)
               ...
               plt.clf()
               print(common_unity_data_complex_y_real_sorted_cut.shape)
               print(common_eda2_data_complex_y_real_sorted_cut.shape)
               plt.plot(common_unity_data_complex_y_real_sorted_cut, common_eda2_data_complex_y_real_sorted_cut,linestyle='None',marker='.')
               #deal with the missing values in the fit
               common_unity_data_complex_y_real_sorted_cut_missing_indices = np.argwhere(~np.isnan(common_eda2_data_complex_y_real_sorted_cut))[:,0]
               #UP TO HERE:::  also need to remove the indices where common_unity_data_complex_y_real_sorted_cut is nan
               #print(common_unity_data_complex_x_real_sorted_cut_missing_indices.shape)
               common_unity_data_complex_y_real_sorted_cut_missing = common_unity_data_complex_y_real_sorted_cut[common_unity_data_complex_y_real_sorted_cut_missing_indices]
               
               print(common_unity_data_complex_y_real_sorted_cut_missing.shape)
               print(results.fittedvalues.shape)
               
               plt.plot(common_unity_data_complex_y_real_sorted_cut_missing, results.fittedvalues, 'r--.', label="OLS fit",linestyle='--',marker='None')
               map_title="Data and fit" 
               plt.xlabel("Expected global-signal response")
               plt.ylabel("Real component of visibility Y pol (Jy)")
               plt.legend(loc=1)
               #optimaise for chan 079
               x_pos = 1100
               y_pos = 0.5e7
               plt.text(x_pos, y_pos, fit_string)
               #plt.ylim([-0.25e7, 1.5e7])
               #plt.xlim([1000, 3000])
               fig_name= "x_y_OLS_%s_%s_pol_%s.png" % (base_name,"y",ms_column)
               figmap = plt.gcf()
               figmap.savefig(fig_name)
               plt.close()
               print("saved %s" % fig_name)
            ...
            #   freq_MHz_fine_chan_index += 14
            #   print("base freq is %s, setting freq_MHz_fine_chan_index to %s " % (base_freq_MHz,freq_MHz_fine_chan_index))
            t_sky_K_array_fine_chan[EDA2_chan_index,freq_MHz_fine_chan_index,timestep,1] = t_sky_K
            t_sky_error_K_array_fine_chan[EDA2_chan_index,freq_MHz_fine_chan_index,timestep,1] = t_sky_error_K
            
            if (np.count_nonzero(~np.isnan(common_eda2_data_complex_y_real_sorted_cut))>2 and np.count_nonzero(~np.isnan(common_unity_data_complex_y_real_sorted_cut))>2):
            
               ###FLAGGING bit 
               #max_deviations = 5 # set in function def
               common_eda2_data_complex_y_real_sorted_cut_std_dev = np.nanstd(common_eda2_data_complex_y_real_sorted_cut)
               max_distance_from_model = max_deviations * common_eda2_data_complex_y_real_sorted_cut_std_dev
               
               ##need to deal with missing values from fit
               common_eda2_data_complex_y_real_sorted_cut_missing_indices = np.argwhere(~np.isnan(common_eda2_data_complex_y_real_sorted_cut))[:,0]
               common_eda2_data_complex_y_real_sorted_cut_missing = common_eda2_data_complex_y_real_sorted_cut[common_eda2_data_complex_y_real_sorted_cut_missing_indices]
               distance_from_model = np.abs(common_eda2_data_complex_y_real_sorted_cut_missing - results.fittedvalues)
               common_eda2_data_complex_y_real_sorted_cut_flagged = np.copy(common_eda2_data_complex_y_real_sorted_cut_missing)
               common_eda2_data_complex_y_real_sorted_cut_flagged[distance_from_model > max_distance_from_model] = np.nan
         
               print(len(common_eda2_data_complex_y_real_sorted_cut_flagged))
               print(len(common_unity_data_complex_y_real_sorted_cut_missing))
            
               #if (len(common_eda2_data_complex_x_real_sorted_cut_flagged)>2 and len(common_unity_data_complex_x_real_sorted_cut_missing)>2):
               if (np.count_nonzero(~np.isnan(common_eda2_data_complex_y_real_sorted_cut_flagged))>2 and np.count_nonzero(~np.isnan(common_unity_data_complex_y_real_sorted_cut_missing))>2):
                  
                  model = sm.OLS(common_eda2_data_complex_y_real_sorted_cut_flagged, common_unity_data_complex_y_real_sorted_cut_missing,missing='drop')
                  results = model.fit()
                  parameters = results.params
                  #print parameters
                  t_sky_K_flagged = parameters[0]
                  ...
                  print("t_sky_K flagged %s y pol is %0.4E +/- %0.04f K" % (ms_column,t_sky_K_flagged,t_sky_error_K_flagged))
                  fit_string = "y=%0.1fx" % t_sky_K_flagged        #t_sky_K=%0.6f K" % (t_sky_jy,t_sky_K)
                  print(fit_string)
                  
                  plt.clf()
                  plt.plot(common_unity_data_complex_y_real_sorted_cut_missing, common_eda2_data_complex_y_real_sorted_cut_flagged,linestyle='None',marker='.')
                  #deal with the missing values in the fit
                  common_unity_data_complex_y_real_sorted_cut_missing_indices = np.argwhere(~np.isnan(common_eda2_data_complex_y_real_sorted_cut_flagged))[:,0]
                  common_unity_data_complex_y_real_sorted_cut_missing = common_unity_data_complex_y_real_sorted_cut_missing[common_unity_data_complex_y_real_sorted_cut_missing_indices]
                  ...
                  #plt.text(x_pos, y_pos, fit_string)
                  #plt.ylim([0, 3.5])
                  fig_name= "x_y_OLS_%s_%s_pol_flagged_%s.png" % (base_name,"y",ms_column)
                  figmap = plt.gcf()
                  figmap.savefig(fig_name)
                  plt.close()
                  print("saved %s" % fig_name)
            ...
            t_sky_K_array_flagged_fine_chan[EDA2_chan_index,freq_MHz_fine_chan_index,timestep,1] = t_sky_K_flagged
            t_sky_error_K_array_flagged_fine_chan[EDA2_chan_index,freq_MHz_fine_chan_index,timestep,1] = t_sky_error_K_flagged
      ###
      #Not using the averaged dat for now just record each timestep and fine chan separately
      ...
      #plt.text(x_pos, y_pos, fit_string)
      #plt.ylim([0, 3.5])
      fig_name= "eda2_response_vs_uvdist_%0.3f_MHz_%s_pol.png" % (freq_MHz_fine_chan,"x")
      figmap = plt.gcf()
      figmap.savefig(fig_name)
      plt.close()
      print("saved %s" % fig_name)
      ...
      #need to guard against having too much flagged data
      number_non_nan = np.count_nonzero(~np.isnan(common_eda2_data_complex_freq_av_x_real_sorted_cut)) 
      if (number_non_nan>2 and len(common_eda2_data_complex_freq_av_x_real_sorted_cut)>2 and len(common_unity_data_complex_freq_av_x_real_sorted_cut)>2):
         ...
         #plt.text(x_pos, y_pos, fit_string)
         #plt.ylim([0, 3.5])
         fig_name= "x_y_OLS_plot_%0.3f_MHz_all_%s_pol.png" % (freq_MHz_fine_chan,"x")
         figmap = plt.gcf()
         figmap.savefig(fig_name)
         plt.close()
         print("saved %s" % fig_name)
      ...
      #   #plt.text(x_pos, y_pos, fit_string)
      #   #plt.ylim([0, 3.5])
      #   fig_name= "x_y_OLS_plot_%0.3f_MHz_%s_pol_%s_flagged.png" % (freq_MHz_fine_chan,"x",EDA2_obs_time)
      #   figmap = plt.gcf()
      #   figmap.savefig(fig_name)
      #   plt.close()
      #   print("saved %s" % fig_name)
...
def plot_autos_and_fit_foregrounds_fine_chan(freq_MHz_list,auto_array_filename,poly_order=5,fine_chans_per_EDA2_chan=27,base_freq_index_offset=0,plotcross=False):
   ...
   #look at some nonautos (cross correlations!)
   if plotcross:
      nonauto_array = np.load("non"+auto_array_filename)
      nonauto_array_timestep_0 = nonauto_array[:,:,0]
      nonauto_array_timestep_0_flatten = np.ndarray.flatten(nonauto_array_timestep_0,order='C')
   
      if math.isclose(base_freq_MHz,50.0):
         nonauto_array_timestep_0_flatten = nonauto_array_timestep_0_flatten[14:]

      plt.clf()
      plt.plot(freq_MHz_fine_chan_array,nonauto_array_timestep_0_flatten)
      map_title="Cross uncal" 
      plt.xlabel("Frequency (MHz)")
      plt.ylabel("Crosscorr uncal")
      #plt.legend(loc=1)
      #plt.text(x_pos, y_pos, fit_string)
      #plt.ylim([0, 3.5])
      fig_name= "non%s.png"  % base_name
      figmap = plt.gcf()
      figmap.savefig(fig_name)
      plt.close()
      print("saved %s" % fig_name)         
...
def plot_t_sky_and_fit_foregrounds_fine_chan(EDA2_chan_list,t_sky_K_array_filename,t_sky_error_K_array_filename,poly_order=5,fine_chans_per_EDA2_chan=27,base_freq_index_offset=0):
   ...
   for pol_index,pol in enumerate(['x','y']):
      #just timestep 0 to compare
      t_sky_K_array_timestep_0 = t_sky_K_array[:,:,0,pol_index]
      t_sky_error_K_array_timestep_0 = t_sky_error_K_array[:,:,0,pol_index]
      t_sky_K_array_timestep_0_flatten = np.ndarray.flatten(t_sky_K_array_timestep_0,order='C')
      t_sky_error_K_array_timestep_0_flatten = np.ndarray.flatten(t_sky_error_K_array_timestep_0,order='C')
      if math.isclose(base_freq_MHz,50.0):
         print("base_freq_MHz is %s not using first 14 t_skys" % base_freq_MHz)
      ...
      for timestep_index in range(n_timesteps):
         #check if not all nans - need to count how many non-nan entries there are so can average the errors correctly (i.e. by sqrt(n) ?)
         t_sky_K_array_timestep = t_sky_K_array[:,:,timestep_index]
         if (~np.isnan(t_sky_K_array_timestep).all()):
            non_nan_counter +=1
      print("non_nan_counter %s" % non_nan_counter)
    
      t_sky_K_array_timestep_av = np.nanmean(t_sky_K_array[:,:,:,pol_index],axis=2)
      ...
      if math.isclose(base_freq_MHz,50.0):
         print("base_freq_MHz is %s not using first 14 t_skys" % base_freq_MHz)
         t_sky_K_array_timestep_av_flatten = t_sky_K_array_timestep_av_flatten[14:]
         t_sky_error_K_array_av_flatten = t_sky_error_K_array_av_flatten[14:]
      ...
      #plt.text(x_pos, y_pos, fit_string)
      #plt.ylim([0, 3.5])
      fig_name= "%s_timestep_av_%s.png"  % (base_name,pol)
      figmap = plt.gcf()
      figmap.savefig(fig_name)
      plt.close()
      print("saved %s" % fig_name)
      ...
      #log fit and subtract
      log_sky_array = np.log10(t_sky_K_array_timestep_av_flatten)
      log_freq_MHz_array = np.log10(np.asarray(freq_MHz_fine_chan_array))
      coefs = poly.polyfit(log_freq_MHz_array, log_sky_array, poly_order)
      ffit = poly.polyval(log_freq_MHz_array, coefs)
      ffit_linear = 10**ffit
      ...
      #log_residual = log_signal_array_short_baselines - log_ffit
      residual_of_log_fit = ffit_linear - t_sky_K_array_timestep_av_flatten
      
      rms_of_residuals = np.sqrt(np.mean(residual_of_log_fit**2))
      print("rms_of_residuals av %s is %0.3f K" % (pol,rms_of_residuals))
...
def manual_flag_and_average_tsky(EDA2_chan_list,t_sky_K_array_filename_fine_chan,t_sky_error_K_array_filename_fine_chan,threshold_sigma=2):
   ...
   for EDA2_chan_index,EDA2_chan in enumerate(EDA2_chan_list):
      ...
      t_sky_K_array_manual_flagged_time_averaged_variance = g((ma-t_sky_K_array_manual_flagged_time_averaged_tile)**2, weights=masked_weights,axis=1)
...
def test_flip(array):
   ...
   if (len(data.shape)>=2 and data.shape[1]==32):
      ...
      print("flipping data")
      flipped_data = np.flip(data,axis=1)
      print(flipped_data[1,:,0])
   else:
      print("data wrong shape")
      print(data.shape)

#array_in = np.empty((10,32,4))
#for index in range(10):
#   array_in[index,:,0] = np.arange(32)
#test_flip(array_in)
#sys.exit()

EDA2_data_dir = '/md0/EoR/EDA2/20200303_data/'   #2020 paper
#the original data are on Marcin's Nimbus4 machine (accessed via nimbus1: /data/2020_03_03_sweep_test/20200303_213605/merged/)
...
run_aoflagger=True
plot_cal = True
#simulate_eda2_with_complex_beams(EDA2_chan_list,lst_hrs_list,nside=32,plot_from_saved=plot_from_saved,EDA2_obs_time_list=EDA2_obs_time_list,sim_unity=sim_unity,sim_pt_source=sim_pt_source,check_figs=check_figs,fine_chan=fine_chan,base_freq_index_offset=base_freq_index_offset)
#input_sky = "gsm"   #zenith_point_source  #unity
...
#reversed all chans after recopying from nimbus4 (8 Aug 2022 12:09pm)
#copy=False    #True
#reverse_ms_fine_chans(EDA2_chan_list=EDA2_chan_list,lst_list=lst_hrs_list,base_freq_index_offset=base_freq_index_offset,copy=copy)
#sys.exit()
...
for ms_column_name in ms_column_name_list:
   ...
   #plotcross=True
   #auto_array_filename_fine_chan = "auto_array_eda2_fine_chan.npy"
   #plot_autos_and_fit_foregrounds_fine_chan(freq_MHz_list,auto_array_filename_fine_chan,base_freq_index_offset=base_freq_index_offset,plotcross=plotcross)
   #sys.exit()
   ...
   #2. average in time and coarse chan and extract signal
   #3. What I (think) I want is average in time only and extract signal per fine chan! 
   #But first need to get all the frequency arrays and stuff consistent - using half the first ms is a spanner in the works!
