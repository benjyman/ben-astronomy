...
parser = OptionParser()
parser.add_option('--aocalibrate', action='store_true',
        help="Switch on to put output model in format compatible with Andres calibrate tool")
...
if options.no_patch:
   if not options.aocalibrate:
	print "++++++++++++++++++++++++++++++++++++++\nCreating weighted srclist for RTS - not mega-patching the sources"
	...
	out_file.close()
        ...
        #for source in weighted_sources[:1]:
        for source in weighted_sources[0:int(options.num_sources)]:
                out_file.write('source {\n')
                out_file.write('  name "%s"\n' % (source.name))
                out_file.write('  component {\n')
                out_file.write('    type point\n')
                position_string_ra_hrs=str(source.ras[0]).split('.')[0]
                position_string_ra_remainder='0.'+str(source.ras[0]).split('.')[1]
                ...
                position_string_dec=ephem_utils.dec2sexstring(source.decs[0],includesign=0,digits=0,roundseconds=1)
