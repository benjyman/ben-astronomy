...
def make_track_off_moon(options):
   ...
   on_moon_file="%s/%s_on_moon_%s.txt" % (on_moon_dir,epoch_ID,centre_chan)
   track_off_moon_outfile_name="track_off_moon_%s_%s.txt" % (epoch_ID,centre_chan)

   track_off_moon_list=[]
   obsid_list=[]
   for line in open(on_moon_file):
      obsid_list.append(line.strip())

   for moon_obsid in obsid_list:
      print moon_obsid
      cmd="cat %s/*.out | grep cotter | grep centre | grep %s | grep -o 'centre.*' | cut -f2- -d' ' > temp_off_moon_%s_%s.txt " % (on_moon_dir,moon_obsid,moon_obsid,centre_chan)
      print cmd
      os.system(cmd)
...
parser.add_option('--epoch_ID',type='string', dest='epoch_ID',default='',help='Epoch ID of observations e.g. --epoch_ID="2018A_01" [default=%default]')


(options, args) = parser.parse_args()

make_track_off_moon(options)
