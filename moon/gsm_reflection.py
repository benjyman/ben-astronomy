...
def gsm_reflection(options):
   
   if options.plot_only:
      #date_time_string_list = ["2015_09_26_11_30_00","2015_09_26_12_00_00","2015_09_26_12_30_00","2015_09_26_13_00_00","2015_09_26_13_30_00","2015_09_26_14_00_00","2015_09_26_14_30_00","2015_09_26_15_00_00","2015_09_26_15_30_00","2015_09_26_16_00_00","2015_09_26_16_30_00","2015_09_26_17_00_00","2015_09_26_17_30_00"]
   ...
   if not plot_only:
         ...
         if options.transit:
            ...
            year=int(date_string.split("_")[0])
            month=int(date_string.split("_")[1])
            day=int(date_string.split("_")[2])
   
            date_in_year = date(year, month, day)
         else:
            date_time_string = options.date_time_string
         ...
         #for date_in_year in rrule(DAILY, dtstart=start_date, until=end_date):
         ...
         if options.transit:
            #need to use pyephem to calcualte transit time of moon
            #later use skyfield
            MWA_pyephem = ephem.Observer()
            MWA_pyephem.lon = mwa_longitude_pyephem
            MWA_pyephem.lat = mwa_latitude_pyephem
            MWA_pyephem.elevation = mwa_elevation
            MWA_pyephem.date = date_in_year.strftime("%Y/%m/%d")
            moon_pyephem = ephem.Moon()
            moon_pyephem.compute(MWA_pyephem)
            print("moon transit time")
            print moon_pyephem.transit_time
            print("moon transit alt")
            print moon_pyephem.transit_alt
         
         
            #Now for skyfield bit:
            date_time_string_from_pyephem =  str(moon_pyephem.transit_time)
            print "date_time_string_from_pyephem"
            print date_time_string_from_pyephem
         
            #print date_time_string
            year=float(date_time_string_from_pyephem.split("/")[0])
            month=float(date_time_string_from_pyephem.split("/")[1])
            day=float(date_time_string_from_pyephem.split("/")[2].split(' ')[0])
            hour=float(date_time_string_from_pyephem.split("/")[2].split(' ')[1].split(':')[0])
            minute=float(date_time_string_from_pyephem.split("/")[2].split(' ')[1].split(':')[1])
            second=float(date_time_string_from_pyephem.split("/")[2].split(' ')[1].split(':')[2])
         
            date_time_string = "%4.0f_%02d_%02d_%02d_%02d_%02d" % (year, month, day, hour, minute, second)
            print "date_time_string"
            print date_time_string
         ...
         MWA = earth + Topos(latitude_degrees=mwa_latitude_dec_deg, longitude_degrees=mwa_longitude_dec_deg ,elevation_m=mwa_elevation)
         moon = planets['moon']
...
usage = 'Usage: gsm_reflection.py [options]'

parser = OptionParser(usage=usage)

parser.add_option('--transit',action='store_true',dest='transit',default=False,help='Just specify dates using --date_string and calculate reflections at Moon transit time e.g. --transit')
parser.add_option('--date_string',type='string', dest='date_string',default='',help='date string in YYYY_MM_DD Used with --transit option (otherwise use --date_time_string) e.g. --date_string=2015_02_25 [default=%default]')
parser.add_option('--plot_only',action='store_true',dest='plot_only',default=False,help='Just make the moon maps from pre-calculated data for one frequency e.g. --plot_only')
parser.add_option('--date_time_string',type='string', dest='date_time_string',default='',help='date_time string in YYYY_MM_DD_HH_mm_ss. Not used with --transit option e.g. --date_time_string=2015_02_25_16_55_00 [default=%default]')


(options, args) = parser.parse_args()

gsm_reflection(options)
