...
# special "fixed AZ scan" for beam measurements
...
def usage():
    ...
    print >> sys.stderr,"fazs [options] <date> <LST_hrs> <east_ZAs> <proj_id> <hrs_to_sched>"
    ...
    print >> sys.stderr,"\t<els>\tin degrees. Comma separated list of elevations for az: "+str(az)
    print >> sys.stderr,"\t      \t\tSweet spot ZAs: 0,13.7,28.3,45.3"
...
opts,args = getopt.getopt(sys.argv[1:], 'esf:u:d:c:m:')
...
zas = numpy.array(map(float,args[2].split(',')))  # degrees
proj_id = args[3]
...
outfile = "%d%02d%02d_cmdfile_FAZS%s_LST%s.txt" % (int(datecomps[0]), int(datecomps[1]), int(datecomps[2]), args[2],args[1])
...
# define ZA ranges for meridian scans
azs = zas*0.0 + az  # default goes east
zas  = abs(zas)
# gain = 1.0    # use default

# calculate the total time of the scans over all DECs and freqs:
stripe_time_hrs = 0.0
for j in range(len(zas)):
...
cmd="single_observation.py --creator=%s --obsname=CORR_MODE_%s_%s --az=0 --el=90 --starttime=%s --stoptime=++48s --freq=69,24 --mode=CORR_MODE_CHANGE --inttime=%s --freqres=%s" % (creator, freqres,timeres,str_starttime,timeres,freqres)
...
while thetime < endtime:
  ...
  for p in range(len(zas)):
    # set the pointing
    az = azs[p]
    el = 90.0 - zas[p]
    ra_hrs = lst_hours
    print >> fpcmd, "# Pointing %d. Az: %g, El: %g" %(p,az,el)
    for f in range(len(cent_freqs)):
        ...
        cmd += " --obsname=FAZ_ZA"+str(zas[p])+"_"+str(cent_freqs[f])
        cmd += " --creator="+creator
        cmd += " --useazel --usegrid= "
        cmd += " --project="+proj_id 
